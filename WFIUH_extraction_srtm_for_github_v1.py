...
def calculate_WFIUH(cat_Name, filePath, savePath, x, y):
    try:
        ...
        grid = Grid.from_raster(filePath + "dir-file/" + cat_Name + '.tif', data_name='catch')
        # N    NE    E    SE    S    SW    W    NW
        ...
        with rasterio.open(filePath + "Manning-v/" + str(cat_Name) + '.tif') as src:
            velocity = src.read(1)
